...
// LfHook is a hook to handle writing to local log files.
type LfHook struct {
	paths     PathMap
	writers   WriterMap
	levels    []logrus.Level
...
func (hook *LfHook) SetFormatter(formatter logrus.Formatter) {
	hook.lock.Lock()
...
func (hook *LfHook) SetDefaultPath(defaultPath string) {
	hook.lock.Lock()
	defer hook.lock.Unlock()
...
func (hook *LfHook) SetDefaultWriter(defaultWriter io.Writer) {
	hook.lock.Lock()
	defer hook.lock.Unlock()
...
func (hook *LfHook) Fire(entry *logrus.Entry) error {
	hook.lock.Lock()
	defer hook.lock.Unlock()
...
func (hook *LfHook) ioWrite(entry *logrus.Entry) error {
	var (
		writer io.Writer
		msg    []byte
		err    error
...
func (hook *LfHook) fileWrite(entry *logrus.Entry) error {
	var (
		fd   *os.File
		path string
		msg  []byte
...
func (hook *LfHook) Levels() []logrus.Level {
