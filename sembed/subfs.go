...
// Otherwise, if fs.FS implements SubFS, Sub returns fsys.Sub(dir).
// Otherwise, Sub returns a new fs.FS implementation sub that,
// in effect, implements sub.Open(name) as fsys.Open(path.Join(dir, name)).
// The implementation also translates calls to ReadDir, fs.ReadFile, and fs.Glob appropriately.
//
...
func (f *subFS) Glob(pattern string) ([]string, error) {
	...
	for i, name := range list {
		...
		if !ok {
			return nil, errors.New("invalid result from inner fsys fs.Glob: " + name + " not in " + f.dir) // can't use fmt in this package
		}
		list[i] = name
	}
