...
func reapChildren(config Config) {
	...
	for {
		...
		for {
			...
			if debug {
				log.Printf(" - Grim reaper cleanup: pid=%d, wstatus=%+v\n",
					pid, wstatus)
			}
...
func reap(disableCheckPid1s ...bool) {
	 ...
	 */
	disableCheckPid1 := false
	if len(disableCheckPid1s) != 0 {
		disableCheckPid1 = disableCheckPid1s[0]
	}

	Start(Config{
		Pid:              -1,
		Options:          0,
		DisablePid1Check: disableCheckPid1,
	})
...
func start(config Config) {
	...
	if !config.DisablePid1Check {
		...
		if 1 != mypid {
			if debug {
				log.Print(" - Grim reaper disabled, pid not 1\n")
			}
			return
		}
	}
