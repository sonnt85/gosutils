...
func TestKb(t *testing.T) {
	...
	if err := smerge.Merge(&b, a); err != nil {
		t.Error(err)
	}
...
func TestNil(t *testing.T) {
	if err := smerge.Merge(nil, nil); err != smerge.ErrNilArguments {
		t.Fail()
	}
}
...
func TestDifferentTypes(t *testing.T) {
	a := simpleTest{42}
	b := 42
	if err := smerge.Merge(&a, b); err != smerge.ErrDifferentArgumentsTypes {
		t.Fail()
	}
}
...
func TestComplexStructWithOverwrite(t *testing.T) {
	...
	if err := smerge.MergeWithOverwrite(&a, b); err != nil {
		t.FailNow()
	}
...
func TestEmbeddedStruct(t *testing.T) {
	...
	for _, test := range tests {
		err := smerge.Merge(&test.dst, test.src)
		if err != nil {
...
func testSlice(t *testing.T, a []int, b []int, e []int, opts ...func(*smerge.Config)) {
	t.Helper()
	bc := b

	sa := sliceTest{a}
	sb := sliceTest{b}
	if err := smerge.Merge(&sa, sb, opts...); err != nil {
		t.FailNow()
	}
	...
	if err := smerge.Merge(&ma, mb, opts...); err != nil {
		t.FailNow()
	}
	...
	if a == nil {
		...
		ma := map[string][]int{}
		mb := map[string][]int{"S": b}
		if err := smerge.Merge(&ma, mb); err != nil {
			t.FailNow()
		}
		if !reflect.DeepEqual(mb["S"], bc) {
...
func TestSlice(t *testing.T) {
	...
	testSlice(t, nil, []int{1, 2, 3}, []int{1, 2, 3}, smerge.WithAppendSlice)
	testSlice(t, []int{}, []int{1, 2, 3}, []int{1, 2, 3}, smerge.WithAppendSlice)
	testSlice(t, []int{1}, []int{2, 3}, []int{1, 2, 3}, smerge.WithAppendSlice)
	testSlice(t, []int{1}, []int{2, 3}, []int{1, 2, 3}, smerge.WithAppendSlice, smerge.WithOverride)
	testSlice(t, []int{1}, []int{}, []int{1}, smerge.WithAppendSlice)
	testSlice(t, []int{1}, nil, []int{1}, smerge.WithAppendSlice)
}
...
func TestMapsWithOverwrite(t *testing.T) {
	...
	if err := smerge.MergeWithOverwrite(&m, n); err != nil {
		t.Errorf(err.Error())
	}
...
func TestMapWithEmbeddedStructPointer(t *testing.T) {
	...
	if err := smerge.Merge(&m, n, smerge.WithOverride); err != nil {
		t.Errorf(err.Error())
	}
...
func TestMergeUsingStructAndMap(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			var err error
			if tc.overwrite {
				err = smerge.Merge(tc.target, *tc.changes, smerge.WithOverride)
			} else {
				err = smerge.Merge(tc.target, *tc.changes)
			}
			if err != nil {
				t.Error(err)
			}
...
func TestMaps(t *testing.T) {
	...
	if err := smerge.Merge(&m, n); err != nil {
		t.Errorf(err.Error())
	}
...
func TestYAMLMaps(t *testing.T) {
	...
	if err := smerge.Merge(&license, thing); err != nil {
		t.Error(err.Error())
	}
...
func TestIfcMapWithOverwrite(t *testing.T) {
	a := ifcTest{13}
	b := ifcTest{42}
	if err := smerge.MapWithOverwrite(&a, b); err != nil {
		t.FailNow()
	}
	if a.I != 42 {
		t.Errorf("b not merged in properly: a.I(%d) != b.I(%d)", a.I, b.I)
...
func TestBackAndForth(t *testing.T) {
	...
	if err := smerge.Map(&m, pt); err != nil {
		t.FailNow()
	}
	var (
		v  interface{}
	...
	bpt := pointerMapTest{}
	if err := smerge.Map(&bpt, m); err != nil {
		t.Error(err)
	}
	if bpt.A != pt.A {
...
func TestEmbeddedPointerUnpacking(t *testing.T) {
	...
	for _, test := range tests {
		pt := test.input
		if err := smerge.MapWithOverwrite(&pt, m); err != nil {
			t.FailNow()
		}
...
func TestTime(t *testing.T) {
	...
	if err := smerge.Merge(&b, dataStruct); err != nil {
		t.FailNow()
	}
	if b.Birth.IsZero() {
	...
	b = structWithTimePointer{}
	if err := smerge.Map(&b, dataMap); err != nil {
		t.FailNow()
	}
	if b.Birth.IsZero() {
...
func TestNestedPtrValueInMap(t *testing.T) {
	...
	if err := smerge.Map(dst, src); err != nil {
		t.FailNow()
	}
...
func TestMergeMapWithInnerSliceOfDifferentType(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			[]func(*smerge.Config){smerge.WithOverride, smerge.WithAppendSlice},
			"cannot append two slices with different type",
		},
		{
			"With override and type check",
			[]func(*smerge.Config){smerge.WithOverride, smerge.WithTypeCheck},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := smerge.Merge(&src, &dst, tc.options...); err == nil || !strings.Contains(err.Error(), tc.err) {
...
func TestMergeSlicesIsNotSupported(t *testing.T) {
	...
	if err := smerge.Merge(&src, &dst, smerge.WithOverride, smerge.WithAppendSlice); err != smerge.ErrNotSupported {
		t.Errorf("expected %q, got %q", smerge.ErrNotSupported, err)
	}
}
