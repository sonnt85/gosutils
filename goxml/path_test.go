...
var tests = []test{
	...
	// bad paths
	{"./bookstore/book[]", errorResult("goxml: path contains an empty filter expression.")},
	{"./bookstore/book[@category='WEB'", errorResult("goxml: path has invalid filter [brackets].")},
	{"./bookstore/book[@category='WEB]", errorResult("goxml: path has mismatched filter quotes.")},
	{"./bookstore/book[author]a", errorResult("goxml: path has invalid filter [brackets].")},
}
...
func TestAbsolutePath(t *testing.T) {
	...
	for _, e := range elements {
		...
		if title == nil || title.Text() != "Everyday Italian" {
			t.Errorf("goxml: absolute path test failed")
		}
