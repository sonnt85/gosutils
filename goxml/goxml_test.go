...
func newDocumentFromString(t *testing.T, s string) *Document {
	...
	if err != nil {
		t.Error("goxml: failed to parse document")
	}
	return doc
}
...
func checkStrEq(t *testing.T, got, want string) {
	t.Helper()
	if got != want {
		t.Errorf("goxml: unexpected result.\nGot:\n%s\nWanted:\n%s\n", got, want)
	}
}
...
func checkIntEq(t *testing.T, got, want int) {
	t.Helper()
	if got != want {
		t.Errorf("goxml: unexpected integer. Got: %d. Wanted: %d\n", got, want)
	}
}
...
func checkBoolEq(t *testing.T, got, want bool) {
	t.Helper()
	if got != want {
		t.Errorf("goxml: unexpected boolean. Got: %v. Wanted: %v\n", got, want)
	}
}
...
func checkElementEq(t *testing.T, got, want *Element) {
	t.Helper()
	if got != want {
		t.Errorf("goxml: unexpected element. Got: %v. Wanted: %v.\n", got, want)
	}
}
...
func checkDocEq(t *testing.T, doc *Document, expected string) {
	...
	if err != nil {
		t.Error("goxml: failed to serialize document")
	}
	if s != expected {
		t.Errorf("goxml: unexpected document.\nGot:\n%s\nWanted:\n%s\n", s, expected)
	}
}
...
`
	...
	if doc.Root() != store {
		t.Error("goxml: root mismatch")
	}
	if len(store.ChildElements()) != 1 || len(store.Child) != 7 {
		t.Error("goxml: incorrect tree structure")
	}
	...
	if len(elements) != 1 || elements[0] != store {
		t.Error("goxml: incorrect SelectElements result")
	}
	element := doc.SelectElement("store")
	if element != store {
		t.Error("goxml: incorrect SelectElement result")
	}
	...
	if attr == nil || attr.Key != "lang" || attr.Value != "en" {
		t.Error("goxml: incorrect SelectAttr result")
	}
	if book.SelectAttrValue("lang", "unknown") != "en" {
		t.Error("goxml: incorrect SelectAttrValue result")
	}
	...
	if attr.Value != "en" {
		t.Error("goxml: incorrect RemoveAttr result")
	}
	...
	if element != title {
		t.Error("goxml: incorrect RemoveElement result")
	}
	element = book.SelectElement("title")
	if element != nil {
		t.Error("goxml: incorrect SelectElement result")
...
func TestImbalancedXML(t *testing.T) {
	...
	for _, c := range cases {
		...
		if err == nil {
			t.Errorf("goxml: imbalanced XML should have failed:\n%s", c)
		}
	}
}
...
func TestDocumentReadNonUTF8Encodings(t *testing.T) {
	...
	err := doc.ReadFromString(s)
	if err != nil {
		t.Fatal("goxml: incorrect ReadFromString result")
	}
}
...
func TestEscapeCodes(t *testing.T) {
	...
	for _, c := range cases {
		...
		s, err := doc.WriteToString()
		if err != nil {
			t.Error("goxml: Escape test produced inocrrect result.")
		}
...
func TestCanonical(t *testing.T) {
	...
	s, err := doc.WriteToString()
	if err != nil {
		t.Error("goxml: WriteSettings WriteTo produced incorrect result.")
	}
...
func TestCopy(t *testing.T) {
	...
	if err != nil {
		t.Error("goxml: incorrect WriteToString result")
	}
	...
	if err != nil {
		t.Error("goxml: incorrect Copy result")
	}

	if s1 != s2 {
		t.Error("goxml: mismatched Copy result")
		t.Error("wanted:\n" + s1)
		t.Error("got:\n" + s2)
	}
	...
	if e1 == nil || e2 == nil {
		t.Error("goxml: incorrect FindElement result")
	}
	if e1 == e2 {
		t.Error("goxml: incorrect FindElement result")
	}
	...
	if s1 == s2 {
		t.Error("goxml: incorrect result after RemoveElement")
	}
}
...
func TestCdata(t *testing.T) {
	...
	for _, test := range tests {
		...
		if err != nil {
			t.Fatal("goxml ReadFromString: " + err.Error())
		}

		tag := doc.FindElement("tag")
		if tag.Text() != test.out {
			t.Fatalf("goxml invalid cdata. Expected: %v. Got: %v\n", test.out, tag.Text())
		}
	}
}
...
<root xmlns="https://root.example.com" xmlns:attrib="https://attrib.example.com" attrib:a="foo" b="bar">
	...
	if len(f) != 2 || f[0] != root || f[1] != child2 {
		t.Error("goxml: failed namespace-uri test")
	}
