...
/*
A Path is a string that represents a search path through an goxml starting
...
traverse the goxml from element to element, while filters are used to narrow
the list of candidate elements at each node.

Although goxml Path strings are structurally and behaviorally similar to XPath
strings (https://www.w3.org/TR/1999/REC-xpath-19991116/), they have a more
limited set of selectors and filtering options.

The following selectors are supported by goxml paths:
...
Below are some examples of goxml path strings.
...
// ErrPath is returned by path functions when an invalid goxml path is provided.
type ErrPath string

// Error returns the string describing a path error.
func (err ErrPath) Error() string {
	return "goxml: " + string(err)
}
